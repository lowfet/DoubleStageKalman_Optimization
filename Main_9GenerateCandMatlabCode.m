...
load('./Mat/AccKMatrix.mat');
load('./Mat/MagKMatrix.mat');
load('./Mat/Qc1andPk1.mat');
load('./Mat/Qc2andPk2.mat');
load('./Mat/MIEstimation.mat');
...
% if exist('STbn','var')
%     
%     fprintf(fid,'STbn = zeros(%d,1);\n',numel(STbn));
%     for rowIndex = 1:numel(STbn)
%         string = char(STbn(rowIndex,1));
%         fprintf(fid,'STbn(%d) = %s;\n',rowIndex,string);
%     end
%     fprintf(fid,'\n');
%     
% end
...
if exist('hmag','var')
    ...
    fprintf(fid,'hmag = zeros(%d,1);\n',numel(hmag));
    for rowIndex = 1:numel(hmag)
        string = char(hmag(rowIndex,1));
        fprintf(fid,'hmag(%d) = %s;\n',rowIndex,string);
    end
    fprintf(fid,'\n');
...
if exist('MI','var')
    ...
    fprintf(fid,'MI = zeros(%d,1);\n',numel(MI));
    for rowIndex = 1:numel(MI)
        string = char(MI(rowIndex,1));
        fprintf(fid,'MI(%d) = %s;\n',rowIndex,string);
    end
    fprintf(fid,'\n');
